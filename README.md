...
> 🖥️ 像素风 · 轻量级 · 跨平台 —— 一键管理你的 **OpenClaw** AI 网关服务

OpenClaw Manager 是一个基于 ASP.NET Core 开发的 Web 管理面板，为 [OpenClaw](https://github.com/yourname/OpenClaw) 提供图形化的一键安装、启动、停止、卸载和实时日志监控能力。采用复古终端美学，让服务管理像敲命令行一样简单直观。

![界面预览](docs/screenshot.png)  <!-- 请替换为实际截图 -->

## ✨ 特性

- 🎮 **像素风仪表盘** —— 致敬经典终端，状态一目了然
...
│                                     │
│  /// SYSTEM LOG                     │
│  > [2026-04-06 07:30:00] [INF] OpenClaw 服务已启动  │
│  > [2026-04-06 07:30:05] [INF] 监听端口 18789        │
└─────────────────────────────────────┘
🚀 快速开始
方式一：下载预编译版本（推荐）
...
dotnet publish OpenClawManager.Api -c Release -r win-x64 --self-contained true /p:PublishSingleFile=true
...
STOP	停止运行中的服务
UNINSTALL	完全删除 OpenClaw 环境（保留配置）
日志窗口	实时滚动显示服务运行日志
自动重连	服务异常退出后自动尝试恢复
⚙️ 配置
...
text
OpenClawManager/
├── OpenClawManager.Api/          # Web API 入口 + 静态前端
│   ├── Controllers/              # REST API（安装/启动/停止/状态）
│   ├── Services/                 # 业务逻辑（安装器、进程管家）
│   ├── Hubs/                     # SignalR 日志推送中心
│   └── wwwroot/                  # 像素风前端（原生 HTML/CSS/JS）
├── OpenClawManager.Core/         # 核心领域与抽象
│   ├── Install/                  # 环境检测、下载、解压
│   └── ProcessClass/             # 进程启动、停止、守护、日志捕获
...
安装失败（网络超时）	检查网络连接，或使用国内镜像（在安装设置中配置 mirror 环境变量）
